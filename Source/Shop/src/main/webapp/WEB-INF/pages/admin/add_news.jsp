...
<script type="text/javascript">
	...
	$(function() {
		$('#insertNewsForm').submit(function() {
			$('#errorShowDiv').html("");
			...
			if (shortContentStr.trim().length < 50) {
				var html_string = '<p style="color:red;font-style:italic;font-size:14px;">Giới thiệu ngắn tối thiểu 50 kí tự!</p>';
				$('#errorShowDiv').html(html_string);
				return false;
			}
			var longContentStr = document
					.getElementById("txtLongContent").value;
			if (longContentStr.trim().length < 100) {
				var html_string = '<p style="color:red;font-style:italic;font-size:14px;">Nội dung bài viết tối thiểu 100 kí tự!</p>';
				$('#errorShowDiv').html(html_string);
				return false;
			}
			// DO STUFF
