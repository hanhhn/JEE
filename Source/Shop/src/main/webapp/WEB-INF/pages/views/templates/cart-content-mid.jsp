	...
	function updateCart(x) {
		...
		$
				...
				.done(
						function(data) {
							var $response = $(data);
							var topCart = $response.filter(
									'#contentCartPanelMainTop').html();
							var contentCart = $response.filter(
									'#contentCartPanelMain').html();
		...
		$
				...
				.done(
						function(data) {
							...
							var message = '<div class="alert alert-success"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>Thông báo! </strong> Đặt hàng thành công <p> <br>Tổng số tiền cho hóa đơn thanh toán: <span class="text-danger">'
									+
<%=((ShoppingCart) request.getSession().getAttribute("ShoppingCart")).getRealFeePurschase() + ""%>
	+ 'đ</span>. Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi. Chúng tôi sẽ liên hệ và chuyển đến bạn trong vòng 24 giờ kể từ lúc này.<a href="../view/home" class="alert-link">Tiếp tục mua sắm</a> </p> </div>';
							$('#InformContentCartPage').append(message);
						})
				.fail(
						function() {
